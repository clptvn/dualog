...
export async function sweepOrphanedPartnerTerminals(dialogsDir, { log = () => {} } = {}) {
  const sessionNames = await listCodexDialogTmuxSessions();
  const ccdSessions = sessionNames.filter((name) => name.startsWith("ccd-"));
  if (ccdSessions.length === 0) {
    return { scanned: 0, terminated: [] };
  }

  const owners = findTerminalOwners(dialogsDir);
  const terminated = [];
  for (const sessionName of ccdSessions) {
    const owner = owners.get(sessionName);
    if (owner?.runnerAlive) continue;

    const reason = owner ? "runner_not_alive" : "unknown_session";
    await terminateTmuxSession(sessionName);
    terminated.push({ session_name: sessionName, reason });
    log(`Terminated orphaned tmux session "${sessionName}" (${reason})`);

    if (owner?.sessionDir && owner.terminal) {
      writeTerminalState(
        owner.sessionDir,
        {
          ...owner.terminal,
  ...
  return { scanned: ccdSessions.length, terminated };
}

async function listCodexDialogTmuxSessions() {
...
function findTerminalOwners(dialogsDir) {
  const owners = new Map();
  if (!dialogsDir || !fs.existsSync(dialogsDir)) return owners;

  for (const entry of fs.readdirSync(dialogsDir, { withFileTypes: true })) {
    if (!entry.isDirectory()) continue;
    const sessionDir = path.join(dialogsDir, entry.name);
    const { current } = readTerminalState(sessionDir);
    if (!current?.session_name) continue;
    const status = readJson(path.join(sessionDir, "status.json"));
    const runnerPid = Number(status?.runner_pid);
    owners.set(current.session_name, {
      sessionDir,
      terminal: current,
      runnerAlive: Number.isSafeInteger(runnerPid) && isProcessAlive(runnerPid),
    });
  }
  return owners;
}
