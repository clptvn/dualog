...
import { getAgentDisplayName, normalizePartnerAgent } from "./shared.mjs";
import {
...
const VALID_GROK_EFFORTS = new Set(GROK_REASONING_EFFORTS);
...
// Grok built-in tool names (see grok --help --tools / --disallowed-tools).
const GROK_READ_DISALLOWED_TOOLS =
  "search_replace,write,spawn_subagent,image_gen,image_edit,image_to_video,reference_to_video";
const GROK_READ_TOOLS =
  "read_file,grep,list_dir,run_terminal_command,web_search,web_fetch,todo_write,ask_user_question";
...
function buildInvocation({
  ...
  const normalizedAgent = normalizePartnerAgent(partnerAgent, "codex");
  ...
  if (normalizedAgent === "grok") {
    // Interactive Grok Build in tmux. Isolated GROK_HOME avoids loading the
    // host's codex-dialog MCP (recursive partner sessions).
    const args = [
      "--cwd",
      projectPath,
      "--always-approve",
    ...
    if (normalizedToolProfile === "read") {
      args.push("--disallowed-tools", GROK_READ_DISALLOWED_TOOLS);
      args.push("--tools", GROK_READ_TOOLS);
    }
    if (model) {
      args.push("--model", model);
    }
    if (reasoningEffort && VALID_GROK_EFFORTS.has(reasoningEffort)) {
      args.push("--reasoning-effort", reasoningEffort);
    }
    // Long bootstrap fits better as argv prompt (same pattern as Codex).
    if (initialPrompt) {
    ...
    return {
      ...
      args,
      env: prepareGrokPartnerEnv(sessionDir),
...
function prepareCodexPartnerEnv(sessionDir) {
  ...
  // Keep the isolated CODEX_HOME lean: no user MCP (avoids recursive dialog
  // servers) and a minimal config so partner turns do not mirror a heavy
  // personal plugin setup into every session directory.
  const partnerConfigPath = path.join(partnerHome, "config.toml");
  if (!fs.existsSync(partnerConfigPath)) {
    fs.writeFileSync(
      partnerConfigPath,
      [
        "# Generated by claude-codex-dialog for an isolated partner session.",
        "# Intentionally minimal: auth is copied, user MCP is not.",
        "",
        "[mcp_servers]",
        "",
      ].join("\n")
...
function prepareGrokPartnerEnv(sessionDir) {
  const sourceHome =
    process.env.GROK_HOME ||
    (process.env.HOME ? path.join(process.env.HOME, ".grok") : null);
  const partnerHome = path.join(sessionDir, "grok-home");
  fs.mkdirSync(partnerHome, { recursive: true });
  ...
  // Lean GROK_HOME: auth only, no MCP servers (prevents recursive codex-dialog).
  const partnerConfigPath = path.join(partnerHome, "config.toml");
  if (!fs.existsSync(partnerConfigPath)) {
    fs.writeFileSync(
      partnerConfigPath,
      [
        "# Generated by claude-codex-dialog for an isolated Grok partner session.",
        "# Auth is copied from the host; MCP servers are intentionally empty.",
        "",
        "[ui]",
  ...
  return {
    GROK_HOME: partnerHome,
    // Avoid picking up a shared leader that might still have host MCP state.
    GROK_USE_LEADER: "0",
  };
}
...
export async function runPartnerCommand({
  ...
  const { command, args, env, usesInitialPrompt } = buildInvocation({
    ...
    sessionName,
    // Codex and Grok accept an initial prompt on the CLI; Claude is pasted after ready.
    initialPrompt:
      normalizedAgent === "codex" || normalizedAgent === "grok" ? bootstrap : null,
  });
...
function isInteractiveReady(agent, snapshot) {
  ...
  if (agent === "grok") {
    const tail = snapshot.slice(-2500);
    if (
      /authenticat/iu.test(tail) &&
      /browser|sign in|login/iu.test(tail) &&
      !/Grok Build|grok-4/iu.test(tail)
    ) {
      return false;
    }
    const hasGrokHeader =
      /\bGrok Build\b/u.test(snapshot) ||
      /\bgrok-\d/iu.test(snapshot) ||
      /\bGrok\b/u.test(snapshot);
    const hasPromptUi =
      snapshot.includes("›") ||
      snapshot.includes("❯") ||
      /shift\+?tab/iu.test(snapshot) ||
      /always-?approve|permission/iu.test(snapshot) ||
      snapshot.includes("Esc to") ||
      /Type a message|Enter to send/iu.test(snapshot);
    // With usesInitialPrompt, "busy" is accepted separately; ready is for paste path.
    return hasGrokHeader && (hasPromptUi || isInteractiveBusy("grok", snapshot));
  }
...
function detectStartupPrompt(agent, snapshot) {
  ...
  if (agent === "grok") {
    if (isInteractiveReady(agent, snapshot)) return null;
    // Project-directory picker / trust-style confirmations if shown.
    if (
      (lowerTail.includes("trust") || lowerTail.includes("project directory")) &&
      (lowerTail.includes("yes") || lowerTail.includes("continue")) &&
      (lowerTail.includes("no") || lowerTail.includes("cancel") || lowerTail.includes("exit"))
    ) {
      return {
        kind: "workspace_trust",
        input: "1",
        description: "project/trust confirmation",
      };
    }
    return null;
  }
