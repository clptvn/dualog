---
description: Have a partner agent audit files or directories for bugs, architecture issues, and correctness
argument-hint: <file or directory paths> [optional: focus area] [optional: rounds:N] [optional: effort:<level>] [optional: model:<id>] [optional: partner:<agent-id>]
...
# /dualog-audit - Comprehensive Code Audit via Codex Dialog MCP Server

Uses the `dualog` MCP server to have the partner agent perform a deep audit of existing code — not changes, but the code as it stands. Reviews architecture, correctness, methodology, potential bugs, unwanted behavior, and more.
...
- **reasoning_effort**: if any argument matches `effort:<level>`, parse it and pass it through unchanged as `reasoning_effort`. Otherwise DO NOT pass it — let the server apply its default of `high`.
- **model**: if any argument matches `model:<name>`, parse it and pass it through unchanged as `model`. Otherwise DO NOT pass it — let the partner use its default model.
...
Instead of sleep-polling `check_messages`, arm a **Monitor** that fires one notification the moment Codex writes its audit. The conversation is appended to `<dialog_dir>/conversation.jsonl`, where `<dialog_dir>` is the **`dialog_dir` returned by the start call**, so a tailed grep on that file is the wake-up signal.
